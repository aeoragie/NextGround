...
public abstract class RepositoryBase
{
    ...
    public async Task<Result<TRow>> QuerySingleOrDefaultAsync<TRow>(string sql, object? parameters = null, int? commandTimeout = null, CancellationToken cancellation = default)
    {
        var stopwatch = Stopwatch.StartNew();
        try
        {
    ...
    public async Task<Result<TRow>> ProcedureSingleOrDefaultAsync<TRow>(ProcedureBase procedure, int? commandTimeout = null, CancellationToken cancellation = default)
    {
        var stopwatch = Stopwatch.StartNew();
        try
        {
            await using var connection = await OpenConnectionAsync(cancellation);
            var parameters = procedure.BuildParameters();
            ...
            if (result == null)
            {
                Logger.Debug("Procedure returned no result in {ElapsedMs}ms: {Procedure}", stopwatch.ElapsedMilliseconds, procedure.ProcedureName);
                return Result<TRow>.Error(ErrorCode.NotFound, "No data found");
            }

            Logger.Debug("Procedure executed in {ElapsedMs}ms: {Procedure}", stopwatch.ElapsedMilliseconds, procedure.ProcedureName);
            return Result<TRow>.Success(result);
        }
        catch (SqlException ex) when (IsTransientError(ex))
        {
            stopwatch.Stop();
            Logger.Warn(ex, "Transient SQL error in {ElapsedMs}ms: {Procedure}", stopwatch.ElapsedMilliseconds, procedure.ProcedureName);
            return Result<TRow>.Error(ErrorCode.TransactionFailed, ex.Message);
        }
        catch (Exception ex)
        {
            stopwatch.Stop();
            Logger.Error(ex, "Procedure failed in {ElapsedMs}ms: {Procedure}", stopwatch.ElapsedMilliseconds, procedure.ProcedureName);
            return Result<TRow>.FromException(ex);
        }
    }
    ...
    public async Task<Result<IEnumerable<TRow>>> QueryAsync<TRow>(string sql, object? parameters = null, int? commandTimeout = null, CancellationToken cancellation = default)
    {
        var stopwatch = Stopwatch.StartNew();
        try
        {
    ...
    public async Task<Result<IEnumerable<TRow>>> ProcedureAsync<TRow>(ProcedureBase procedure, int? commandTimeout = null, CancellationToken cancellation = default)
    {
        var stopwatch = Stopwatch.StartNew();
        try
        {
            ...
            stopwatch.Stop();
            var count = result.TryGetNonEnumeratedCount(out var c) ? c : -1;
            Logger.Debug("Procedure returned {Count} rows in {ElapsedMs}ms: {Procedure}", count, stopwatch.ElapsedMilliseconds, procedure.ProcedureName);
            return Result<IEnumerable<TRow>>.Success(result);
        }
        catch (SqlException ex) when (IsTransientError(ex))
        {
            stopwatch.Stop();
            Logger.Warn(ex, "Transient SQL error in {ElapsedMs}ms: {Procedure}", stopwatch.ElapsedMilliseconds, procedure.ProcedureName);
            return Result<IEnumerable<TRow>>.Error(ErrorCode.TransientError, ex.Message);
        }
        catch (Exception ex)
        {
            stopwatch.Stop();
        ...
        CancellationToken cancellation = default)
    {
        ...
        catch (SqlException ex) when (IsTransientError(ex))
        {
            stopwatch.Stop();
            Logger.Warn(ex, "Transient SQL error in {ElapsedMs}ms: {Sql}", stopwatch.ElapsedMilliseconds, TruncateSql(sql));
            return Result<int>.Error(ErrorCode.TransientError, ex.Message);
        }
        catch (Exception ex)
        {
            stopwatch.Stop();
        ...
        CancellationToken cancellation = default)
    {
        ...
        try
        {
            ...
            stopwatch.Stop();
            Logger.Debug("Procedure affected {AffectedRows} rows in {ElapsedMs}ms: {Procedure}", affectedRows, stopwatch.ElapsedMilliseconds, procedure.ProcedureName);
            return Result<int>.Success(affectedRows);
        }
        catch (SqlException ex) when (IsTransientError(ex))
        {
        ...
        {
            stopwatch.Stop();
            Logger.Error(ex, "Procedure execute failed in {ElapsedMs}ms: {Procedure}", stopwatch.ElapsedMilliseconds, procedure.ProcedureName);
            return Result<int>.FromException(ex);
        }
    }
    ...
    public async Task<Result<TResult>> ExecuteInTransactionAsync<TResult>(
        Func<DbConnection, DbTransaction, Task<TResult>> operation,
    ...
    public async Task<Result<int>> ExecuteInTransactionAsync(
        Func<DbConnection, DbTransaction, Task<int>> operation,
        IsolationLevel isolationLevel = IsolationLevel.ReadCommitted,
        CancellationToken cancellation = default)
    {
        ...
        CancellationToken cancellation = default)
    {
        ...
        while (true)
        {
            ...
            if (result.ResultData.Code != ErrorCode.TransientError || attempt >= retryCount)
            {
                if (attempt > 1)
                {
