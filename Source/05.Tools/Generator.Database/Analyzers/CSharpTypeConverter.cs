    ...
    public static class CSharpTypeConverter
    {
        ...
        private static (string CSharpType, ValueType ValueType) GetBaseCSharpType(string mssqlType, string? defineType)
        {
            ...
            return normalizedType switch
            {
                ...
                "timestamp" => ("byte[]", ValueType.Vector), // 추가: SQL Server timestamp
                "rowversion" => ("byte[]", ValueType.Vector), // 추가: SQL Server rowversion
                ...
                _ when normalizedType.StartsWith("char(") => ("string", ValueType.String), // 추가: char(n)
                _ when normalizedType.StartsWith("nchar(") => ("string", ValueType.String), // 추가: nchar(n)
        ...
        public static string GenerateMemberValue(ColumnSchema column)
        {
            var (csharpType, valueType) = GetCSharpType(column.DataType, column.UserDefinedType, column.IsNullable);
            var defaultValue = GetDefaultValue(valueType, column.IsNullable);
            return $"public {csharpType} {column.ColumnName} {{ get; set; }} = {defaultValue};";
        }
    }
}
